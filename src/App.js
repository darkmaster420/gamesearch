...
const GameSearchApp = () => {
  ...
  const extractGamePoster = (game) => {
    // For gog-games, use the image field if available
    if (game.siteType === 'goggames' && game.image) {
      return game.image;
    }
  ...
  const getSiteBadgeStyle = (game) => {
    switch (game.siteType) {
      case 'skidrow':
      ...
      case 'goggames':
        return 'bg-gradient-to-r from-purple-500 to-violet-500 text-white shadow-lg shadow-purple-500/30';
      default:
        return 'bg-gradient-to-r from-gray-500 to-gray-600 text-white';
    }
  };

  const getSiteName = (game) => {
    switch (game.siteType) {
      case 'skidrow':
        return 'SKIDROW';
      case 'freegog':
  ...
  const getStatsDisplayName = (siteName) => {
    switch (siteName) {
          ...
          <div onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex flex-wrap gap-3 justify-center">
              {[
                ...
                { value: 'goggames', label: 'GOG-Games', color: 'from-purple-500 to-violet-500' }
              ].map(option => (
                <button
                  key={option.value}
                  type="button"
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <div className="flex flex-wrap gap-3 justify-end">
                      {Object.entries(stats).map(([site, count]) => (
                        <span key={site} className={`px-3 py-1 rounded-full text-xs font-bold ${getStatsStyle(site)}`}>
                          {getStatsDisplayName(site)}: {count}
                        </span>
                      ))}
                    </div>
                  </div>
                    ...
                    <div className="bg-gradient-to-br from-gray-800/80 to-gray-900/80 backdrop-blur-sm rounded-3xl border border-gray-700/50 overflow-hidden hover:border-cyan-400/50 transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl hover:shadow-cyan-500/20 hover:from-gray-700/80 hover:to-gray-800/80">
                      ...
                      <div className="relative h-64 overflow-hidden">
                        ...
                        <div className="absolute top-4 right-4">
                          <span className={`px-3 py-1 rounded-full text-xs font-bold backdrop-blur-md ${getSiteBadgeStyle(game)}`}>
                            {getSiteName(game)}
                          </span>
                        </div>
                        ...
                        {/* GOG-Games specific info badges */}
                        {game.siteType === 'goggames' && (
                          <div className="absolute bottom-4 left-4 flex flex-wrap gap-2">
                            {game.version && (
                              <span className="px-2 py-1 rounded-full text-xs font-bold bg-gradient-to-r from-indigo-500 to-purple-500 text-white backdrop-blur-md shadow-lg">
                                v{game.version}
                              </span>
                            )}
                            {game.size && (
                              <span className="px-2 py-1 rounded-full text-xs font-bold bg-gradient-to-r from-amber-500 to-orange-500 text-white backdrop-blur-md shadow-lg">
                                {game.size}
                              </span>
                            )}
                          </div>
